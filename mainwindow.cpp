...
static int partnumber; //工件序号
static int picturenumber; //图片序号
...
static QDir qDir1("C:/Users/GDZ/Desktop/measure_win/Measure/image/origin");
static QDir qDir2("C:/Users/GDZ/Desktop/measure_win/Measure/image/measured");
//设置表头
static QStringList headers = QObject::trUtf8("工件序号,槽序号,主宽度,侧宽度1,侧宽度2,状态").simplified().split(",");
...
//    ui->stopButton->setIcon(icon2);
    ...
    theSelection = new QItemSelectionModel(
        theModel); //创建数据选择模型时需要传递一个数据模型变量作为其参数。数据选择模型theSelection是theModel的项数据选择操作。
    ...
    ui->tableView->setColumnWidth(0, 75);//设置固定宽度
    ui->tableView->setColumnWidth(1, 75);//设置固定宽度

    // 每个元件数据栏
    //数据模型和选择模型的创建，以及与视图组件的关联等设置
    theModel2 = new QStandardItemModel(0, FixedColumnCount, this); //数据模型
    theSelection2 = new QItemSelectionModel(
        theModel2); //创建数据选择模型时需要传递一个数据模型变量作为其参数。数据选择模型theSelection是theModel的项数据选择操作。
    theModel2->setHorizontalHeaderLabels(headers);
    //创建数据模型和选择模型后，为TableView组件设置数据模型和选择模型
    ui->tableView_2->setModel(theModel2); //设置数据模型
    ...
    ui->tableView_2->horizontalHeader()->setDefaultAlignment(Qt::AlignLeft|Qt::AlignVCenter);
    ui->tableView_2->horizontalHeader()->setSectionResizeMode(0, QHeaderView::Fixed);//对第0列单独设置固定宽度
    ui->tableView_2->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Fixed);//对第0列单独设置固定宽度
    ui->tableView_2->setColumnWidth(0, 60);//设置固定宽度
    ui->tableView_2->setColumnWidth(1, 45);//设置固定宽度
    ...
    ui->lcdPictureNumber->setDigitCount(5);
...
void MainWindow::on_measure_clicked()
{
    ...
    Mat imgg,imgg2;
    char p[100];
    sprintf(p, "C:/Users/GDZ/Desktop/measure_win/Measure/workpiece-measure/img/34/test/%d.bmp", picturenumber+1);
    imgg=imread(p);
    double mainwidth;
    double sidewidth1;
    double sidewidth2;
    //开始拍照
    MainMeasure(imgg,imgg2,mainwidth,sidewidth1,sidewidth2);
    QImage img,img2;
    img=MatToQImage(imgg);
    img2=MatToQImage(imgg2);
    ...
    ui->picture_show_label_2->setPixmap(QPixmap::fromImage(img2)); // img转为像素图，放在picture_show_label中
    ui->picture_show_label_3->setPixmap(QPixmap::fromImage(img)); // img转为像素图，放在picture_show_label中
    ui->picture_show_label_4->setPixmap(QPixmap::fromImage(img2)); // img转为像素图，放在picture_show_label中

    //保存图片
    char a[100];
    sprintf(a, "C:/Users/GDZ/Desktop/measure_win/Measure/image/origin/%d_%d.bmp", partnumber,picturenumber);
    char b[100];
    sprintf(b, "C:/Users/GDZ/Desktop/measure_win/Measure/image/measured/%d_%d.bmp", partnumber,picturenumber);
    img.save(a);
    img2.save(b);
    ...
    QList<QStandardItem *> aItemList2; //容器类
    QStandardItem *aItem11 = new QStandardItem(QString::number(partnumber));
    QStandardItem *aItem22 = new QStandardItem(QString::number(picturenumber));
    QStandardItem *aItem33 = new QStandardItem(QString::number(mainwidth));
    QStandardItem *aItem44 = new QStandardItem(QString::number(sidewidth1));
    QStandardItem *aItem55 = new QStandardItem(QString::number(sidewidth2));
    QStandardItem *aItem66 = new QStandardItem(quality);
    aItemList2 << aItem11 << aItem22 << aItem33 << aItem44 << aItem55 << aItem66;
    theModel2->insertRow(theModel2->rowCount(), aItemList2); //插入一行，需要每个Cell的Item
    QModelIndex curIndex2 = theModel2->index(theModel2->rowCount() - 1, 0); //创建最后一行的ModelIndex
    theSelection2->clearSelection(); //清空选择项
    theSelection2->setCurrentIndex(curIndex2, QItemSelectionModel::Select); //设置刚插入的行为当前选择行
...
//    slotnumber=8;
    ...
    fTimer->stop();
    fTimer->setInterval(10000 / slotnumber); //时间间隔  10s÷slotnumber
...
void MainWindow::on_stopButton_clicked() {
    ...
    // 数据标准下拉菜单使能
    ui->slotBox->setEnabled(true);
    ui->mainwidthBox->setEnabled(true);
    ui->sidewidthBox->setEnabled(true);
    ui->mainerrorBox->setEnabled(true);
    ui->sideerrorBox->setEnabled(true);
...
void MainWindow::on_save_Button_clicked() {
    QString dirPath =
        QFileDialog::getSaveFileName(NULL, QStringLiteral("ImageSaveAs"), "/home/gdz/图片/demo.png",
...
void MainWindow::on_read_Button_clicked() {
    QString dirPath = QFileDialog::getOpenFileName(this, tr("选择图像"), "/home/gdz/图片/demo.png",
...
void MainWindow::on_openfileButton_clicked() {
    ...
    {
        QTextStream aStream(
            &aFile); // QTextStream与IO读写设备结合，为数据读写提供了一些方便的方法的类，以流的方式读写文件
        while (!aStream.atEnd()) {
...
//            theModel->setData(index, int(Qt::AlignLeft |
//            Qt::AlignVCenter),Qt::TextAlignmentRole);//对齐方式(水平方向在左，垂直方向在中)
//        }
//    }
//}
...
void MainWindow::on_savedataButton_clicked() {
    QString curPath = "/home/gdz/文档/222.txt";
    //调用打开文件对话框选择一个文件
    QString aFileName =
        QFileDialog::getSaveFileName(this, tr("选择一个文件"), curPath, "井斜数据文件(*.txt);;所有文件(*.*)");
...
void MainWindow::on_toolButton_clicked()
{
    ...
    if (my_serialport->open(QIODevice::ReadWrite)) //用ReadWrite的模式尝试打开串口
    {
        ...
        // 数据标准下拉菜单使能
        ui->slotBox->setEnabled(false);
        ui->mainwidthBox->setEnabled(false);
        ui->sidewidthBox->setEnabled(false);
        ui->mainerrorBox->setEnabled(false);
        ui->sideerrorBox->setEnabled(false);
...
void MainWindow::on_tableView_2_clicked(const QModelIndex &index)
{
    ...
    if (index.isValid()){
        ...
        int id=index.row();
        QString id1 = theModel2->item(id, 0)->text();
        QString id2= theModel2->item(id, 1)->text();

        char a[100];
        sprintf(a, "C:/Users/GDZ/Desktop/measure_win/Measure/image/origin/%d_%d.bmp", id1.toInt(),id2.toInt());
        char b[100];
        sprintf(b, "C:/Users/GDZ/Desktop/measure_win/Measure/image/measured/%d_%d.bmp",id1.toInt(),id2.toInt());
        QImage img;
        QImage img2;
        img.load(a);
        img2.load(b);
